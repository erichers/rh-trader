...
- Standing autofix: `POST /api/bots/autofix` (`{ dry_run?: true }`) plus a 20-min worker tick (paper, kill off). Puts / leftover equity / covered-call sells / non-allowlist 0–1 → `mode=observe` **and `enabled=0`**, and **`last_result` is cleared** so the Bots banner drops. Long-call LEAPS → `full_auto` + `option` + `call` + `buy` (far expiry kept). New-entry exits clamp sl≤10 / tp 20 / trail 8–15 (prefer 10); `hold_overnight` + `hold_over_weekend` true. Never widens live open-position stops. Health: `autofix: { lastRun, lastFixedCount, lastError }`. The Mac `scripts/autofix-bots.py` 20-min bridge can retire after this merge. Bots UI `issues()` only counts `last_result` error/skip while the bot is enabled (allowlist skip on a disabled bot is not an issue).
