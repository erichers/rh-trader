...
**Use `auto`, not `full_auto`.** Same-symbol book ($10k) and 25% concentration are hard in both modes. `full_auto` only skips the 40-orders/day throttle. First watched session = `auto`.
...
2. Kill switch **off**.
3. Global mode **Auto** (not Full-Auto).
4. Leave the three watch stubs enabled if you want signals: **Mean-Revert Watch**, **Quiet Range Scout**, **Vol-Regime MR**. They must stay watch-only.
5. Enable only the bots you intend to watch (Donchian / Momentum Day / Opening Range are the META-stack trio).
6. Optional: `POST /api/bots/:id/evaluate` once after the open to force a pass.

## Watch

- **Orders**: no `draft` / `staged` / `placed` / `vetoed` rows from the three watch stubs. Signals may log; that is not an order.
- **Same name**: if Donchian + Momentum + ORB fire on one symbol, the third ticket must **veto** on symbol book / concentration, not pass as another $10k ticket. Same if a bot is accidentally set to `full_auto`.
...
## Success criteria for merging PR #6
...
- [ ] A same-symbol stack that would exceed **$10k** (or 25% of equity) was **vetoed** in `auto`.
- [ ] Kill switch stopped new buys; you could still flatten.
- [ ] No one switched to Robinhood live. No real-money order left the box.
- [ ] You used **`auto`**. If anything ran `full_auto`, book/concentration still held (orders/day may have been skipped — that is the only remaining bypass).
