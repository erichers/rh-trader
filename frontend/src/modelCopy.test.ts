...
describe('Jev desk copy', () => {
  it('translates skip codes into desk language', () => {
    ...
    assert.equal(jevSkipLabel('rail'), 'hard rail');
    assert.equal(jevSkipLabel('error'), 'error');
  ...
  it('falls back to the health cadence bands', () => {
    assert.deepEqual(JEV_CADENCE_FALLBACK.map((b) => b.dte), ['0 to 3', '4 to 14', '15 to 179', '180 and out']);
    assert.deepEqual(JEV_CADENCE_FALLBACK.map((b) => b.every), ['5 min', '20 min', '60 min', '4 hr']);
  });
});
