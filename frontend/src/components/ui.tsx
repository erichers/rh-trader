...
export const DOCS: Record<string, string> = {
  ...
  alerts: 'Two AI loops feed this: Groq scans incoming news fast and cheap every couple hours; anything it flags as tradeable is escalated to Kimi for a deeper take and a "trade" alert linked to the relevant bots. Kimi also runs a daily review for trend/event highlights.',
  ...
  mode: 'Observe = log only (no orders). Cautious = every order staged for your one-click approval. Auto = bots execute when rules + risk pass. Full-Auto = bots execute with the soft sizing caps OFF (position/concentration/orders-per-day no longer veto — still shown); only the kill switch + daily-loss breaker + no-crypto/long-only still apply.',
  env: 'Paper (Alpaca) for testing with fake money; Live (Robinhood) trades your real agentic account. Switching swaps the whole portfolio view. Kill switch + risk limits apply in both.',
