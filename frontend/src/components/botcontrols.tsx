...
export const MODE_INFO: Record<string, { label: string; short: string; tip: string; cls: string }> = {
  ...
  full_auto: { label: 'Full-Auto', short: 'full', cls: 'red', tip: 'FULL AUTONOMY — auto-executes like Auto. Same-symbol book ($10k) and 25% concentration stay HARD (they used to be bypassed — that let META stack). Only the orders/day throttle is soft. Monday paper arming mode. Real money if Live.' },
};
...
export const MODE_LEGEND =
  'Bot modes (increasing autonomy): Observe = watch only, no orders. Cautious = every order is staged for your one-click approval. Auto = trades automatically when rules + risk pass (exits auto-enforced). Full-Auto = same hard rails as Auto ($10k book, 25% concentration, kill switch, 50% daily breaker) — only the orders/day throttle is skipped. Monday paper arming mode is Full-Auto. Enable the bot (ON), then pick a mode — you can change either from any tab.';
